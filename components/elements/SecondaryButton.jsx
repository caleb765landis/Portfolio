...
export default function SecondaryButton({ link, text, icon, type, download=false, downloadPath="/images/profile.jpg" }) {
    let componentToRender = null

    if (download) {
        componentToRender = <DownloadButton link={link} text={text} downloadPath={downloadPath} icon={icon} />
    } else {
...
function DownloadButton({ link, text, downloadPath, icon }) {
    return (
        <a href={link ? link : "/"} download={downloadPath}>
            ...
            {icon ? <IconImports.FontAwesomeIcon icon={icon} size="lg" className="pl-2" /> : null}
        </a>
    )
}
